...
export default function UpdateEventStatus({ event }: { event: any }) {
    ...
    return (
        <div className="flex flex-col xl:flex-row xl:items-center gap-3 xl:gap-4 py-3 xl:p-2 rounded w-full xl:w-max">
            {!hasTickets && (
                ...
                <fetcher.Form method="post" action={`/my-events/${event.slug}/status`} className="m-0 col-span-1 sm:w-auto">
                    ...
                    >
                        {isPast ? (
                            ...
                            <>
                                <RiTicketLine className="size-4 shrink-0" />
                                <span className="hidden sm:inline">Close Sales</span>
